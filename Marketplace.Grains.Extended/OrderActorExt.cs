...
{
    [Reentrant]
    public class OrderActorExt : ExtendedTransactionActor, IOrderActorExt
    {
        ...
        public async Task<object> Checkout(TransactionContext tctx, object input)
        {
            ...
            var paymentActor = GrainFactory.GetGrain<IExtendedTransactionActor>(paymentActorPartition, "Marketplace.Grains.Extended.PaymentActorExt");
            await paymentActor.Execute(tctx, new FunctionCall("ProcessPayment", invoice, typeof(PaymentActorExt)));
            ...
            //Console.WriteLine("Order grain {0} -- Checkout process ended for customer {1} -- Order id is {2}",
            ...
            if (statusResp.Count - relevantItems.Count > 0)
            {
                ...
                foreach (var item in differenceItems)
                {
                    ...
                    noopsTasks.Add(stockActor.Execute(
                        tctx, new FunctionCall("Noop", null, typeof(StockActorExt))));
                }
