...
{
    [Reentrant]
    public class OrderActorOrleans : Grain, IOrderActorOrleans
    {
        ...
        private readonly ILogger<OrderActorOrleans> _logger;

        private static readonly decimal[] emptyArray = Array.Empty<decimal>();

        public OrderActorOrleans(ILogger<OrderActorOrleans> _logger)
        {
            this.nextOrderId = 1;
        ...
        public async Task Init()
        {
           ...
           // _logger.LogInformation("Order grain {0} activated: #stock grains {1} #order grains {2} #payment grains {3} ",
          //      this.orderActorId, nStockPartitions, nOrderPartitions, nPaymentPartitions);
        }
        public async Task Checkout(CheckoutParameter cp)
        {
           ...
           // Console.WriteLine("From the initial amount of items: {0}, where {1} items unavailable. We need to perform {1} amount of NoOps"
            ...
            var paymentActor = GrainFactory.GetGrain<IPaymentActorOrleans>(paymentActorPartition, "Marketplace.Grains.Orleans.Actors.PaymentActorOrleans");
